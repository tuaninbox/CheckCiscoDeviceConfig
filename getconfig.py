...
##### GOOD #####
        ...
        if site and find: #1 site + find keywords
            ...
            for i in reader:
                # if i["Name"] == str(args.site):
                if i["Name"] == str(site):
                    print(searchconfig(i["Name"],i["Host"],username if i["Username"] == "" else i["Username"],
                            password if i["Password"] == "" else i["Password"],cmd,findstring))
        # elif not args.site and args.find: # all sites + find keywords
        elif not site and find: # all sites + find keywords
            # print("find command in all sites")
            with concurrent.futures.ThreadPoolExecutor() as executor:
                results=[executor.submit(searchconfig,i["Name"],i["Host"],username if i["Username"] == "" else i["Username"],
                            password if i["Password"] == "" else i["Password"],cmd,findstring) for i in reader if i['Name'][0:1] != "#"]
